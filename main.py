...
async def _stream(companies: list[str], market: str, country: str):
    try:
        ...
        winner = max(results, key=lambda x: x[2].probability)[0] if results else None
        ...
        # Run competitor analysis for all companies; losers get winning strategy
        ...
        for (company, _, _), comp_data in zip(results, comp_results):
            ...
            yield _e({
                ...
                "data": {
                    ...
                    "winning_strategy": comp_data.get("winning_strategy", "") if is_loser else "",
                }
            })
